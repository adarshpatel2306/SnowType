...
let gameTime = 30 * 1000; // default to 30s
window.timer = null;
...
function newGame() {
  ...
  const userTime = parseInt(timeInput);
  if (!isNaN(userTime) && userTime > 0) {
    gameTime = userTime * 1000;
  } else {
    gameTime = 30 * 1000;
  }
...
function getAccuracy() {
  const allTypedLetters = [...document.querySelectorAll('.letter.correct, .letter.incorrect')];
  const correctLetters = allTypedLetters.filter(l => l.classList.contains('correct')).length;
  const totalTyped = allTypedLetters.length;

  if (totalTyped === 0) return 0;
  return (correctLetters / totalTyped) * 100;
}
